...
## Четвёртая волна: закрытие дыр + сетевая устойчивость

Более глубокий проход «закрыть все дыры, чтобы не повторялись» + отдельный аудит
сетевых сбоев. Каждая находка воспроизведена и покрыта тестом/гейтом CI.

**Дыры безопасности/DoS (backend, `selftest_hardening.py` — 12/12):**
- Бэкдор тест-оплаты: `/v1/billing/mock/{checkout,confirm}` теперь 404, если
  `PAY_PROVIDER≠mock` — после перехода на боевого провайдера нельзя бесплатно
  подтвердить свой pending-платёж.
- Гостевой тред поддержки ключевался САМИМ bearer-токеном и этот ключ отдавался в
  `/v1/admin/support/threads` → утечка токенов работникам поддержки (угон сессии).
  Теперь — хеш токена (`g:sha256`).
- `/v1/node/register` возвращал уже выданный per-node секрет по публичному id →
  восстановление секрета любого узла. Теперь секрет всегда свежий (ротация).
- Минт ценности (`/v1/admin/grant` вкл. lifetime, `/v1/admin/user/{id}/balance`)
  перенесён со scope `users` на `billing` — саппорт-работник не начислит безлимит.
- Rate-limit на `anon`/`login`/`recover`/`discount-check` (было: безлимитная фарма
  аккаунтов; брутфорс пароля + CPU-усиление scrypt; оракул купонов).
- TTL-очистка PoW-челленджей (утечка памяти при `POW_BITS=0`) и `node_sessions`;
  middleware лимита тела запроса (256 KiB, тело читалось до auth); лимиты размеров
  сообщения/метки/кода и числа сообщений в треде.

**Сетевая устойчивость (ничего не виснет/не течёт):**
- Flutter `api_client`: КАЖДЫЙ HTTP-вызов через обёртку с таймаутом 15с (был только
  у `ping`) — чёрнодырный бэкенд больше не подвешивает future и не залипает `busy`;
  `solvePow` ограничен (был бесконечный цикл); `downloadMbps` — сторож тишины вместо
  зависающего `await for` (была утечка HttpClient); таймаут на чтение тела traffic;
  reentry-guard'ы на тики; `refreshFromBackend` — single-flight; `applyRules`/вотчдог
  через единый `_connecting` (не двоят sing-box).
- JS (app/site/bridge): `fetch` с AbortController+таймаут (иначе `bootstrap` не
  завершался); расширение — корреляция native-сообщений по `_id`, лимит кадра в
  `host.py`.
- Установщики: `--max-time`/`--retry` на curl.
- **Устойчивое (проверено, багов нет):** Go-движок (парсеры пакетов bounds-checked,
  все сетевые чтения с дедлайнами, карты ограничены), `db.py` (транзакции/rollback),
  крипта манифеста/узлов (`compare_digest`), control-plane скрипты (таймауты).

**Защита от повторения:** новый CI-джоб `web` (синтаксис всего JS + inline-скриптов
`.dc.html` + коннектор-тесты); backend/control-plane регресс-тесты и так в CI по
glob `selftest_*.py`. Проверено: backend **25/25**, control-plane **4/4**, Go
кросс-сборка+vet, весь Python/JS, i18n-паритет 55×391.
